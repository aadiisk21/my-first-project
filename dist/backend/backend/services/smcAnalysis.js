...
class SMCAnalysis {
    ...
    analyzeMarketData(marketData, timeframe = '1h') {
        ...
        const smcLevels = this.calculateSMCLevels(marketStructure, volumeProfile, orderBlocks, fairValueGaps, recentData);
        return {
    ...
    calculateVolumeProfile(data) {
        ...
        const poc = levels.reduce((max, level) => level.volume > max.volume ? level : max, levels[0]);
     ...
     */
    identifyOrderBlocks(data, volumeProfile) {
        const orderBlocks = [];
        ...
        for (let i = 3; i < data.length - 3; i++) {
            ...
            const hasRejection = nextCandles.some(nextCandle => (candle.close > candle.open && nextCandle.high < candle.high) ||
            ...
            if (isHighVolume && isStrongMove && hasRejection) {
                ...
                const strength = this.calculateOrderBlockStrength(candle, previousCandles, nextCandles, orderBlockType);
                if (strength > 0.6) {
                    const { takeProfit, stopLoss } = this.calculateOrderBlockTargets(candle, orderBlockType, volumeProfile);
                    orderBlocks.push({
                        price: candle.close,
        ...
        return orderBlocks.filter((ob, index, self) => index === 0 ||
     ...
     */
    calculateSMCLevels(structure, volumeProfile, orderBlocks, fairValueGaps, data) {
        ...
        const imbalancedHighs = this.findImbalancedPriceLevels(data, 'HIGH', volumeProfile);
        const imbalancedLows = this.findImbalancedPriceLevels(data, 'LOW', volumeProfile);
        ...
        const { buyLiquidity, sellLiquidity } = this.calculateLiquidityZones(data, volumeProfile, swingHigh, swingLow);
        return {
            swingHigh,
            swingLow,
            imbalancedLows,
    ...
    findHigherHighs(swingPoints, highs) {
        const higherHighs = [];
    ...
    findHigherLows(swingPoints, lows) {
        const higherLows = [];
        let currentMax = Infinity;
    ...
    findLowerHighs(swingPoints, highs) {
        const lowerHighs = [];
    ...
    findLowerLows(swingPoints, lows) {
        const lowerLows = [];
        let currentMin = Infinity;
    ...
    determineTrend(higherHighs, higherLows, currentPrice) {
    ...
    getLastSwingPoint(swingPoints, highs, lows) {
    ...
    calculateValueArea(levels, totalVolume) {
    ...
    calculateOrderBlockStrength(candle, previous, next, type) {
        let strength = 0;
        // Volume strength
    ...
    calculateOrderBlockTargets(candle, type, volumeProfile) {
    ...
    findImbalancedPriceLevels(data, type, volumeProfile) {
        const imbalanced = [];
        const avgVolume = data.reduce((sum, d) => sum + d.volume, 0) / data.length;
        const volumeThreshold = avgVolume * this.volumeThreshold;
    ...
    calculateLiquidityZones(data, volumeProfile, swingHigh, swingLow) {
     ...
     */
    generateSMCSignals(smcLevels, currentPrice, timeframe = '1h') {
    ...
    generateLiquidityZones(smcLevels, timeframe) {
        const zones = [];
    ...
    calculateDynamicStopLoss(currentPrice, signal, smcLevels) {
    ...
    calculateDynamicTakeProfit(currentPrice, signal, smcLevels) {
        const { swingHigh, swingLow } = smcLevels;
...
exports.SMCAnalysis = SMCAnalysis;
//# sourceMappingURL=smcAnalysis.js.map