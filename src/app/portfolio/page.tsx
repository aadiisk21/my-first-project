    ...
    async function loadPortfolio() {
      try {
        ...
        if (mounted && json?.success && Array.isArray(json.data?.assets)) {
          ...
          const total = assetsFromApi.reduce((sum, asset) => sum + asset.value, 0);
          const pnl = assetsFromApi.reduce((sum, asset) => sum + asset.unrealizedPnL, 0);
          setTotalValue(total);
          setTotalPnL(pnl);
      ...
      const pnl = mockAssets.reduce((sum, asset) => sum + asset.unrealizedPnL, 0);
      setTotalValue(total);
      setTotalPnL(pnl);
      setTotalPnLPercent((pnl / (total - pnl)) * 100);
      setIsLoading(false);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Your crypto holdings and performance</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="trading-card p-4">
          ...
          <div className="text-3xl font-bold text-foreground mt-2">${totalValue.toLocaleString('en-US', { maximumFractionDigits: 2 })}</div>
          <div className="text-xs text-muted-foreground mt-2">USD equivalent</div>
        </div>
        <div className="trading-card p-4">
          <div className="text-sm text-muted-foreground">Unrealized P&L</div>
          <div className={`text-3xl font-bold mt-2 ${totalPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ${totalPnL.toLocaleString('en-US', { maximumFractionDigits: 2 })}
          </div>
          <div className={`flex items-center mt-2 text-xs ${totalPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {totalPnL >= 0 ? <ChevronUpIcon className="h-4 w-4" /> : <ChevronDownIcon className="h-4 w-4" />}
            <span>{totalPnLPercent.toFixed(2)}%</span>
          </div>
        </div>
        <div className="trading-card p-4">
          <div className="text-sm text-muted-foreground">Total Holdings</div>
          <div className="text-3xl font-bold text-foreground mt-2">{assets.length}</div>
          <div className="text-xs text-muted-foreground mt-2">Active cryptocurrencies</div>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-right py-3 px-4 text-muted-foreground font-medium">Unrealized P&L</th>
                <th className="text-right py-3 px-4 text-muted-foreground font-medium">24h Change</th>
              </tr>
            </thead>
            <tbody>
              {assets.map((asset) => (
                <tr key={asset.symbol} className="border-b border-border hover:bg-secondary transition-colors">
                  ...
                  <td className="text-right py-3 px-4 text-foreground">${asset.currentPrice.toLocaleString('en-US', { maximumFractionDigits: 2 })}</td>
                  ...
                  <td className="text-right py-3 px-4 text-muted-foreground">
                    ${asset.entryPrice.toLocaleString('en-US', { maximumFractionDigits: 2 })}
                  </td>
                  <td className="text-right py-3 px-4">
                    <div className={asset.unrealizedPnL >= 0 ? 'text-green-600' : 'text-red-600'}>
                      <div className="font-semibold">
                        ${asset.unrealizedPnL.toLocaleString('en-US', { maximumFractionDigits: 2 })}
                      </div>
                  ...
                  <td className="text-right py-3 px-4">
                    <div className={`flex items-center justify-end gap-1 ${asset.changePercent24h >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {asset.changePercent24h >= 0 ? <ChevronUpIcon className="h-4 w-4" /> : <ChevronDownIcon className="h-4 w-4" />}
