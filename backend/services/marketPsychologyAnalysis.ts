...
interface CrowdBehavior {
  ...
  smartMoneyActivity: 'ABSORPTION' | 'DISTRIBUTION' | 'STOP_HUNT' | 'LIQUIDITY_GRAB';
}
...
interface SocialSignal {
  platform: 'TWITTER' | 'REDDIT' | 'TELEGRAM' | 'DISCORD' | 'YOUTUBE' | 'NEWS';
...
interface PsychologicalLevel {
  level: 'CAPITULATION_DISTRIBUTION' | 'TREND_FOLLOWING' | 'RATIONAL_ANALYSIS' | 'EMOTIONAL_TRADING';
  efficiency: number;
  winRate: number;
  riskAdjusted: number;
}

export class MarketPsychologyAnalysis {
  private fearGreedThresholds = {
    extremeFear: 20,
    fear: 40,
    neutral: 60,
    greed: 80,
  ...
  private volumeProfileWindow = 20;
  private sentimentSmoothing = 0.3;
  private crowdBehaviorThreshold = 0.7;
  ...
  analyzeMarketPsychology(
    ...
    contrarianSignals: ContrarianIndicator[];
    psychologicalLevels: PsychologicalLevel[];
  } {
   ...
   */
  private analyzeCrowdBehavior(data: MarketData[], fgi: FearGreedIndex): CrowdBehavior {
  ...
  private calculateMarketSentiment(
    ...
    const volatility = this.calculateVolatility(
      data.slice(-50).map(d => d.high),
      data.slice(-50).map(d => d.low)
    );
   ...
   */
  private generateContrarianSignals(
    data: MarketData[],
    crowdBehavior: CrowdBehavior,
    sentiment: MarketSentiment
  ): ContrarianIndicator[] {
    const signals: ContrarianIndicator[] = [];
   ...
   */
  private analyzePsychologicalEfficiency(
    data: MarketData[],
    crowdBehavior: CrowdBehavior
  ): PsychologicalLevel[] {
    const efficiencyLevels: PsychologicalLevel[] = [];
    const timeframes = ['1h', '4h', '1d'];

    timeframes.forEach(timeframe => {
      const timeframeData = this.getTimeframeData(data, timeframe);
      const efficiency = this.calculatePsychologicalEfficiencyForTimeframe(
        timeframeData,
        crowdBehavior,
        timeframe
      );
  ...
  private calculateBuySellPressure(data: MarketData[]): { buyPressure: number; sellPressure: number } {
  ...
  private analyzeVolumeProfile(volumes: number[]): VolumeProfile {
    ...
    let distribution: VolumeProfile['distribution'];
    let buyPressure = 0.5;
  ...
  private calculateVolumeEfficiency(volumes: number[]): number {
  ...
  private detectPriceAction(data: MarketData[]): CrowdBehavior['priceAction'] {
  ...
  private detectSmartMoneyActivity(data: MarketData[], fgi: FearGreedIndex): CrowdBehavior['smartMoneyActivity'] {
  ...
  private calculateHerdConviction(volumes: number[], closes: number[], fgi: FearGreedIndex): number {
  ...
  private calculateVolumeAgreement(volumes: number[]): number {
  ...
  private classifyCrowdBehavior(
    fgi: FearGreedIndex,
    volumeProfile: VolumeProfile,
    priceAction: CrowdBehavior['priceAction'],
    smartMoneyActivity: CrowdBehavior['smartMoneyActivity']
  ): CrowdBehavior['herdBehavior'] {
  ...
  private processSocialSentiment(socialSignals?: SocialSignal[]): number {
    if (!socialSignals || socialSignals.length === 0) return 0;
  ...
  private analyzeOptionsFlow(data: MarketData[]): {
    putCallRatio: number;
    callRatio: number;
  } {
    const recentData = data.slice(-30);
  ...
  private calculateContrarianProbability(
    crowdBehavior: CrowdBehavior,
    sentiment: MarketSentiment,
    signalType: ContrarianIndicator['signalType']
  ): number {
  ...
  private calculateContrarianTargets(data: MarketData[], signalType: 'CONTRARIAN_BUY' | 'CONTRARIAN_SELL'): number[] {
    const currentPrice = data[data.length - 1].close;
    const recentData = data.slice(-50);
  ...
  private calculatePsychologicalEfficiencyForTimeframe(
    data: MarketData[],
    crowdBehavior: CrowdBehavior,
    timeframe: string
  ): PsychologicalLevel {
    ...
    let level: PsychologicalLevel['level'];
  ...
  private simulatePsychologicalTrading(
    data: MarketData[],
    crowdBehavior: CrowdBehavior,
    timeframe: string
  ): number {
  ...
  private calculateRiskAdjustedReturn(data: MarketData[], crowdBehavior: CrowdBehavior): number {
    const volatility = this.calculateVolatility(
      data.map(d => d.high),
      data.map(d => d.low)
    );
  ...
  private estimateMarketCap(data: MarketData[]): number {
  ...
  private calculateBitcoinDominance(data: MarketData[]): number {
    // Mock calculation - would use external data in production
    return 45 + Math.random() * 10; // Mock BTC dominance percentage
  }

  private applyExternalFactors(baseIndex: number, external: ExternalFactors): number {
